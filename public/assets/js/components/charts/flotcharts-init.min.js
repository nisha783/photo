...
function initCrosshairTracking() {
  ...
  var c,
    d = $("#crosshairTracking .legendLayer text tspan"),
    p = (d.each(function () {
  ...
  function t() {
    ...
    if (!(o.x < i.xaxis.min || o.x > i.xaxis.max || o.y < i.yaxis.min || o.y > i.yaxis.max)) for (var a = l.getData(), t = 0; t < a.length; ++t) {
      for (var e = a[t], n = 0; n < e.data.length && !(e.data[n][0] > o.x); ++n);
      var s = e.data[n - 1],
        r = e.data[n],
        r = null == s ? r[1] : null == r ? s[1] : s[1] + (r[1] - s[1]) * (o.x - s[0]) / (r[0] - s[0]);
      d.eq(t).text(e.label.replace(/=.*/, "= " + r.toFixed(2)));
    }
  }
  $("#crosshairTracking").bind("plothover", function (o, i, a) {
    c = i, p = p || setTimeout(t, 50);
...
function initStackedBar() {
  ...
  l(), $(".stackControls button").click(function (o) {
    ...
    o.preventDefault(), n = -1 != $(this).text().indexOf("Bars"), s = -1 != $(this).text().indexOf("Lines"), r = -1 != $(this).text().indexOf("steps"), l();
  });
}
